...
def network_build(link_filename, link_filename_no_weight, node_filename, db_object_GHTorrent, sql_table_name, start_time, end_time):
    ...
    for index_i in range(num):
        for index_j in range(index_i + 1, num):
            ...
            res_links_no_weight.append([repos_id[index_i], repos_id[index_j], 1, 'undirected'])
            ...
            if len(res_links) > 100000000:
                ...
                res_links = []
            if len(res_links_no_weight) > 100000000:
                util.print_list_row_to_csv(link_filename_no_weight, res_links_no_weight, 'a')
                res_links_no_weight = []
...
if __name__ == '__main__':
    ...
    for year in range(2011, 2019):
        for month in range(1, 13):
            ...
            link_filename_no_weight = "data_2.0\\month\\" + "links_no_weight_" + str(year) + "_" + str(month) + ".csv"
            ...
            util.print_list_row_to_csv(link_filename_no_weight, [['Source', 'Target', 'Weight', 'Type']], 'w')
            ...
            # 不同项目之间相同的开发者数
            network_build(link_filename, link_filename_no_weight, node_filename, db_object_GHTorrent, sql_table_name, start_time, end_time)
            # 执行结束时间
